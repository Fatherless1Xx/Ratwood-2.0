...
/datum/sex_action/tailpegging_anal/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	if(HAS_TRAIT(target, TRAIT_TINY) && !HAS_TRAIT(user, TRAIT_TINY))
		user.visible_message(span_warning("[user] forces [user.p_their()] tail into [target]'s tiny butt!"))
		...
		user.visible_message(span_warning("[user] tries and fails to insert [user.p_their()] tiny tail into [target]'s butt."))
	else
...
/datum/sex_action/tailpegging_anal/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	if(!HAS_TRAIT(target, TRAIT_TINY) && HAS_TRAIT(user, TRAIT_TINY))
		user.visible_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()] tries to fuck [target]'s ass with [user.p_their()] tail, unsuccessfully."))
...
/datum/sex_action/tailpegging_anal/on_finish(mob/living/carbon/human/user, mob/living/carbon/human/target)
	if(!HAS_TRAIT(target, TRAIT_TINY) && HAS_TRAIT(user, TRAIT_TINY))
		user.visible_message(span_warning("[user] stops trying to insert [user.p_their()] tiny tail into [target]'s butt."))
	else
		user.visible_message(span_warning("[user] pulls [user.p_their()] tail out of [target]'s ass."))
